"""Layer 3 pure logic -- match rows by key and find duplicates.
...
Pure computation only (no filesystem, architecture.md's code-style rule);
``run`` lives in ``run.py`` and export/report I/O in ``output.py``. Built on
``src/utils/keys.py`` so layers 3 and 4 can never drift apart on pairing
semantics.
"""
...
from src.utils.keys import duplicate_mask, key_frame

LAYER_NAME = "layer_3_compare_rows"
...
def row_verdict(classification: dict[str, set]) -> Verdict:
    ...
    makes it ``success``. Per D1, the pipeline CONTINUEs past
    ``completed-with-differences`` -- only ``failed`` stops it.
    """
